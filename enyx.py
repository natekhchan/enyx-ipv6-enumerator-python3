...
if os.path.isfile("/usr/bin/snmpwalk"):
    ...
    command = f'snmpwalk -c {community} -v {version} {ip} iso.3.6.1.2.1.4.34.1.3.2.16|cut -d "." -f 13-28 | cut -d " " -f 1'
    ...
    def ip(IPS):
        for i in range(0, output2):
            ...
            step2 = ''.join(step1)
            step3 = step2.split(".")
            step4 = list(map(int, step3))  # Convert map to list
            step5 = list(map(hex, step4))  # Convert map to list
            s = 0
            for j in step5:
